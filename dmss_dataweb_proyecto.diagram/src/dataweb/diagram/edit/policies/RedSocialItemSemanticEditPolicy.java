...
public class RedSocialItemSemanticEditPolicy extends DatawebBaseItemSemanticEditPolicy {
	...
	*/
	public RedSocialItemSemanticEditPolicy() {
		super(DatawebElementTypes.RedSocial_2014);
	}
	...
	*/
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (DatawebVisualIDRegistry.getVisualID(incomingLink) == PaginaPantallaRedsocialEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (DatawebVisualIDRegistry.getVisualID(outgoingLink) == RedSocialPaginapantallaEditPart.VISUAL_ID) {
